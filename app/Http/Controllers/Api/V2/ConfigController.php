...
class ConfigController extends Controller
{
    ...
    {
        $activated_social_login_list = IsyolSetting::whereIn('type', ['facebook_login', 'google_login', 'twitter_login'])->get();
        return response()->json($activated_social_login_list);
    }

    public function isyol_settings(Request $request)
    {
        $isyol_settings = IsyolSetting::whereIn('type', explode(',', $request->keys))->get()->toArray();
        ...
        // $settings = array_merge($isyol_settings, $new_array);

        return response()->json($isyol_settings);
    }
}
