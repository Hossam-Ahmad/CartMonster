...
@section('content')
    ...
    <div class="aiz-titlebar text-left mt-2 mb-3">
        <div class="row align-items-center">
            <div class="col-auto">
                <h1 class="h3 text-light">{{ translate('All Auction products') }}</h1>
            </div>
                                ...
                                <td class="text-right">
                                    @if ($product->auction_start_date > strtotime('now'))
                                        <a class="btn btn-primary btn-icon btn-circle btn-sm"
                                            href="{{ route('auction_product_edit.admin', ['id' => $product->id, 'lang' => env('DEFAULT_LANGUAGE')]) }}"
                                            title="{{ translate('Edit') }}">
                                            <i class="las la-edit"></i>
                                        </a>
                                    @endif
                                    <a class="btn btn-soft-success btn-icon btn-circle btn-sm"
                                        href="{{ route('auction-product', $product->slug) }}" target="_blank"
                                        title="{{ translate('View Products') }}">
                                        <i class="las la-eye"></i>
                                    </a>
                                    <a class="btn btn-soft-info btn-icon btn-circle btn-sm"
                                        href="{{ route('product_bids.admin', $product->id) }}" target="_blank"
                                        title="{{ translate('View All Bids') }}">
                                        <i class="las la-gavel"></i>
                                    </a>
                                    <a href="#" class="btn btn-soft-danger btn-icon btn-circle btn-sm confirm-delete"
                                        data-href="{{ route('auction_product_destroy.admin', $product->id) }}"
