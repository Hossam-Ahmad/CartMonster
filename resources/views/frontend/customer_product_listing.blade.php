...
@if (isset($category_id))
    @php
        $meta_title = \App\Models\Category::find($category_id)->getTranslation('meta_title');
        $meta_description = \App\Models\Category::find($category_id)->getTranslation('meta_description');
    @endphp
@elseif (isset($brand_id))
    @php
        $meta_title = \App\Models\Brand::find($brand_id)->getTranslation('meta_title');
        $meta_description = \App\Models\Brand::find($brand_id)->getTranslation('meta_description');
    @endphp
@else
    @php
        $meta_title = get_setting('meta_title', null, App::getLocale());
        $meta_description = get_setting('meta_description', null, App::getLocale());
    @endphp
@endif
            ...
            <form class="" id="search-form" action="" method="GET">
                                        ...
                                        <ul class="list-unstyled">
                                            ...
                                            @else
                                                ...
                                                @if (\App\Models\Category::find($category_id)->parent_id != 0)
                                                    <li class="mb-2">
                                                        <a class="text-reset fs-14 fw-600"
                                                            href="{{ route('customer_products.category', \App\Models\Category::find(\App\Models\Category::find($category_id)->parent_id)->slug) }}">
                                                            <i class="las la-angle-left"></i>
                                                ...
                                                <li class="mb-2">
                                                    <a class="text-reset fs-14 fw-600"
                                                        href="{{ route('customer_products.category', \App\Models\Category::find($category_id)->slug) }}">
                                                        <i class="las la-angle-left"></i>
                                                ...
                                                @foreach (\App\Utility\CategoryUtility::get_immediate_children_ids($category_id) as $key => $id)
                                                    <li class="ml-4 mb-2">
                                                        <a class="text-reset fs-14"
                                                            href="{{ route('customer_products.category', \App\Models\Category::find($id)->slug) }}">{{ \App\Models\Category::find($id)->getTranslation('name') }}</a>
                                                    </li>
                                                @endforeach
                                            @endif
                                        </ul>
                        ...
                        <ul class="breadcrumb bg-transparent p-0">
                            ...
                            @if (isset($category_id))
                                <li class="text-dark fw-600 breadcrumb-item">
                                    <a class="text-reset"
                                        href="{{ route('customer_products.category', \App\Models\Category::find($category_id)->slug) }}">"{{ \App\Models\Category::find($category_id)->getTranslation('name') }}"</a>
                                </li>
                            @endif
                        </ul>
                ...
                <div class="form-group ml-auto mr-0 w-200px d-none d-md-block">
                    ...
                    <select class="form-control form-control-sm aiz-selectpicker" name="condition"
                        ...
                        <option value="new"
                            @isset($condition) @if ($condition == 'new') selected @endif
                        @endisset>{{ translate('New') }}</option>
                    <option value="used"
                        @isset($condition) @if ($condition == 'used') selected @endif
                    @endisset>{{ translate('Used') }}</option>
            </select>
        </div>
