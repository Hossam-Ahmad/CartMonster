                ...
                @foreach ($coupons as $coupon)
                    ...
                    @php
                        if ($coupon->user->user_type != 'admin') {
                            ...
                            $name = get_setting('website_name', null, app()->getLocale());
                        }
                    @endphp
                            ...
                            <div class="rounded bg-white h-100 overflow-hidden" style="border-color: #2d3748 !important">
                                <div class="row no-gutters h-100">
                                                    ...
                                                    @foreach ($products as $key => $product)
                                                        @if ($product != null && $key < 3)
                                                            <a href="{{ route('product', $product->slug) }}"
                                                                class='col-4 text-center text-reset mb-3' target="_blank">
                                                                <img class="img-fit mw-100 h-90px h-sm-120px h-lg-80px h-xl-100px h-xxl-130px mb-2 rounded"
                                                                ...
                                                                <div class="lh-1-2">
                                                                    <span
                                                                        class="fw-600 text-primary fs-16">{{ home_discounted_base_price($product) }}</span>
                                                                    @if (home_base_price($product) != home_discounted_base_price($product))
                                                                        <del
                                                                            class="fw-500 opacity-50 fs-14">{{ home_base_price($product) }}</del>
                                                                    @endif
                                                                </div>
                                                            </a>
                                                        @endif
                                        ...
                                        <div class="col-md-4 bg-primary d-flex">
                                            <div class="align-self-center p-3 flex-grow-1 text-center">
                                                @if ($coupon->discount_type == 'amount')
                                                    <p class="fs-20 fw-700 mb-1">{{ single_price($coupon->discount) }}
                                                        {{ translate('OFF') }}</p>
                                                @else
                                                    <p class="fs-20 fw-700 mb-1">{{ $coupon->discount }}%
                                                        {{ translate('OFF') }}</p>
                                                @endif
                                                ...
                                                <a class="btn bg-white fw-700"
                                                    @if ($coupon->user->user_type != 'admin') href="{{ route('shop.visit', $shop->slug) }}"
                                                @else
                                                    href="{{ route('inhouse.all') }}" @endif>
                                    ...
                                    @else
                                        <div class="col-md-8 text-dark align-self-center">
                                            <div class="px-4 py-3">
                                                ...
                                                <span class="h5 text-center d-block m-auto bg-soft-info p-3 rounded">
                                                    @if ($coupon->discount_type == 'amount')
                                                        {{ translate('Min Spend ') }}
                                                        {{ single_price($order_discount->min_buy) }}
                                                        {{ translate('from') }} {{ $name }}
                                                        {{ translate('to get') }}
                                                        {{ single_price($coupon->discount) }}
                                                        {{ translate('OFF on total orders') }}
                                                    @else
                                                        {{ translate('Min Spend ') }}
                                                        {{ single_price($order_discount->min_buy) }}
                                                        {{ translate('from') }} {{ $name }}
                                                        {{ translate('to get') }} {{ $coupon->discount }}%
                                                        {{ translate('OFF on total orders') }}
                                                    @endif
                                                </span>
                                            </div>
                                        ...
                                        <div class="col-md-4 bg-primary d-flex">
                                            <div class="align-self-center p-3 flex-grow-1 text-center">
                                                <p class="fs-20 fw-700 mb-1">{{ translate('Max Discount') }}:
                                                    {{ single_price($order_discount->max_discount) }}</p>
                                                <span class="fs-16 d-block mb-3">
                                                    {{ translate('Code') }}:
                                                    <span class="fw-600">{{ $coupon->code }}</span>
                                                </span>
